...
import { LayoutDashboard, Users, Download, ClipboardList, Shield, LogOut, Table2, FileText, ScrollText, Upload, History, Sliders, FlaskConical, ClipboardCheck, Bug, BarChart3, BookOpen, Rocket, ChevronsLeft, ChevronsRight, UserCog, Stethoscope, Activity, AlertCircle, Ruler, Settings2, Hash, ShieldCheck, Trash2, Gauge, ClipboardEdit } from "lucide-react";
...
const NAV: { to: string; label: string; icon: any; perm?: string; perms?: string[] }[] = [
  ...
  { to: "/selections", label: "Master Seleksi", icon: ClipboardList, perm: PERMISSIONS.CANDIDATE_VIEW },
  ...
  { to: "/data-belum-lengkap", label: "Data Belum Lengkap", icon: AlertCircle, perm: PERMISSIONS.CANDIDATE_VIEW },
  { to: "/peserta-tanpa-no-test", label: "Peserta Tanpa No Test", icon: Hash, perm: PERMISSIONS.NO_TEST_VIEW },
  { to: "/import-nomor-tes", label: "Import Nomor Tes", icon: Hash },
  { to: "/setting-hari-h", label: "Setting Hari-H", icon: Settings2, perm: PERMISSIONS.HARI_H_SETTINGS_VIEW },
  { to: "/bypass-review", label: "Review Bypass", icon: ShieldCheck, perm: PERMISSIONS.HARI_H_BYPASS_VIEW },
  { to: "/master-juknis", label: "Master Parameter Juknis", icon: Ruler, perm: PERMISSIONS.JUKNIS_VIEW },
  { to: "/rekap-aplikasi", label: "Rekap APLIKASI", icon: Table2, perm: PERMISSIONS.CANDIDATE_VIEW },
  { to: "/laporan-tahap", label: "Laporan Tahap", icon: FileText, perm: PERMISSIONS.CANDIDATE_VIEW },
  { to: "/resume-casis", label: "Resume Casis", icon: ScrollText, perm: PERMISSIONS.CANDIDATE_VIEW },
  { to: "/exports", label: "Export Rekap", icon: Download, perms: [PERMISSIONS.EXPORT_XLSX, PERMISSIONS.EXPORT_PDF] },
  ...
  { to: "/rule-simulator", label: "Rule Simulator", icon: FlaskConical, perm: PERMISSIONS.RULE_SIMULATOR_VIEW },
  ...
  { to: "/user-management", label: "Manajemen Pengguna", icon: UserCog, perm: PERMISSIONS.USER_MANAGEMENT_VIEW },
  { to: "/medical-subteams", label: "Master Subtim", icon: Stethoscope, perm: PERMISSIONS.MASTER_SUBTIM_VIEW },
];
...
export function AppShell() {
  ...
  const isPatientOnly =
    !isSuperAdmin &&
    roles.length > 0 &&
    roles.every((r) => r === "peserta" || r === "casis");
  const PATIENT_ALLOWED = new Set<string>([
    "/my-anamnesis",
    "/help-center",
  ]);
  const PATIENT_NAV = [
  ...
  return (
    <div className="flex min-h-screen bg-[hsl(210_40%_96%)]">
      ...
      >
        <div className={`${collapsed ? "px-2 py-4 justify-center" : "px-5 py-5"} border-b border-white/10 flex items-center gap-3`}>
          <img src={rikkesLogo} alt="Logo RIKKES TNI AU" width={40} height={40} className="h-10 w-10 object-contain shrink-0 drop-shadow" loading="eager" />
          {!collapsed && (
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-1">
          {(isPatientOnly ? PATIENT_NAV : NAV).filter((n) => {
        ...
        <div className="p-2 border-t border-white/10 space-y-2">
          ...
          {!collapsed && (
            <div className="px-2 text-[11px]">
              ...
              <div className="opacity-70 truncate capitalize">{(roles[0] ?? "viewer").replace(/_/g, " ")}</div>
            </div>
          )}
          <Button
            variant="destructive"
