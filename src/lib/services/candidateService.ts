...
export function createCandidateLocal(input: any) {
  const db = getDb() as any;
  if (!input.selection_id) throw new Error("selection_id wajib");
  const now = nowIso();
  const tn = String(input.test_number ?? "").trim();
  if (tn) {
    const dup = findDuplicateTestNumberLocal({ selectionId: input.selection_id, testNumber: tn });
  ...
  const candidate = {
    ...
    rank: input.rank ?? "",
    nrp_nip: input.nrp_nip ?? "",
    unit_position: input.unit_position ?? input.unit ?? "",
    pok_korp: input.pok_korp ?? "",
    panda: input.panda ?? "",
    group_name: input.group_name ?? input.kelompok ?? "",
    birth_place: input.birth_place ?? "",
    birth_date: input.birth_date ?? "",
    phone: input.phone ?? "",
    address: input.address ?? "",
    test_number: tn,
    temporary_id: tn ? "" : generateTemporaryId(db),
    test_number_status: tn ? "assigned" : "pending",
    no_test_missing: !tn,
    registration_notes: input.registration_notes ?? "",
    is_deleted: false,
    created_at: now,
    updated_at: now,
  };
