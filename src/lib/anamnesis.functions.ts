...
async function loadFormAndRoles(localDataApi: any, examId: string, userId: string) {
  const [{ data: form, error: e1 }, { data: rolesData, error: e2 }] = await Promise.all([
    localDataApi.from("medical_history_forms").select("*").eq("exam_id", examId).maybeSingle(),
    localDataApi.from("user_roles").select("role").eq("user_id", userId),
  ]);
...
async function syncExamSectionStatus(
  localDataApi: any,
  examId: string,
...
export const patientSaveDraft = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { form, roles } = await loadFormAndRoles(localDataApi, data.examId, userId);
    if (!canPatientEdit(form, roles))
      throw new Error("Tidak diizinkan menyimpan draft anamnesis pada status ini");
    ...
    const { error } = form
      ? await localDataApi.from("medical_history_forms").update(payload).eq("id", form.id)
      : await localDataApi.from("medical_history_forms").insert(payload);
    if (error) throw new Error(error.message);
    await writeAudit(localDataApi, {
      userId,
      action: "anamnesis.patient.save_draft",
      examId: data.examId,
      candidateId: data.candidateId,
      after: { status: "Draft Peserta" },
    });
    return { ok: true };
  });
...
export const patientSubmitAnamnesis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await localDataApi
      ...
      .eq("id", form.id);
    if (error) throw new Error(error.message);
    await syncExamSectionStatus(localDataApi, data.examId, "Submitted");
    await writeAudit(localDataApi, {
      userId,
      action: "anamnesis.patient.submit",
      examId: data.examId,
      candidateId: data.candidateId,
      after: { status: "Submitted Peserta" },
    });
    return { ok: true };
  });
...
async function doctorMutate(
  ...
  if (!canDoctorReview(form, roles))
    throw new Error("Anda tidak berhak mereview anamnesis ini saat ini");
  ...
  await writeAudit(localDataApi, {
    ...
    examId: data.examId,
    candidateId: data.candidateId,
    before: { status: form.anamnesis_workflow_status },
    after: patch,
  });
}
...
export const doctorRequestClarification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await syncExamSectionStatus(localDataApi, data.examId, "Revision");
    return { ok: true };
  });
...
export const doctorSubmitReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isDokterUmum(roles) && !isAdminLike(roles))
      throw new Error("Hanya dokter umum yang dapat menandatangani");
    if (
      form.anamnesis_workflow_status !== "Clear Dokter" &&
      form.anamnesis_workflow_status !== "Ada Catatan Dokter"
    ) {
    ...
    await writeAudit(localDataApi, {
      userId,
      action: "anamnesis.doctor.submit_review",
      examId: data.examId,
      candidateId: data.candidateId,
      after: { signed_at: now },
    });
...
export const registrationUpdateIdentity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { localDataApi, userId } = context;
    const { form, roles } = await loadFormAndRoles(localDataApi, data.examId, userId);
    if (!canRegistrationEditIdentity(form, roles))
      throw new Error("Tidak diizinkan mengubah identitas pada status ini");
    const payload = {
    ...
    const { error } = form
      ...
      : await localDataApi
          .from("medical_history_forms")
          .insert({ ...payload, anamnesis_workflow_status: "Draft Peserta" });
    if (error) throw new Error(error.message);
    await writeAudit(localDataApi, {
      userId,
      action: "anamnesis.registration.update_identity",
      examId: data.examId,
      candidateId: data.candidateId,
      after: { identity_data_json: data.identity_data_json },
    });
    return { ok: true };
  });
...
export const adminReturnToDraft = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { form, roles } = await loadFormAndRoles(localDataApi, data.examId, userId);
    if (!form) throw new Error("Form belum ada");
    if (!canAdminReturnToDraft(form, roles))
      throw new Error("Tidak diizinkan mengembalikan ke draft");
    const now = new Date().toISOString();
    const { error } = await localDataApi
      .from("medical_history_forms")
      .update({
      ...
      .eq("id", form.id);
    if (error) throw new Error(error.message);
    await syncExamSectionStatus(localDataApi, data.examId, "Draft");
    await writeAudit(localDataApi, {
      userId,
      action: "anamnesis.admin.return_to_draft",
      examId: data.examId,
      candidateId: data.candidateId,
      before: { status: form.anamnesis_workflow_status },
      after: { status: "Draft Peserta", reason: data.reason },
    });
    return { ok: true };
  });
