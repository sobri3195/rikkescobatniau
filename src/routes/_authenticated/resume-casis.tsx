...
function ResumeCasis() {
  ...
  const load = useCallback(async () => {
    ...
    const [c, e, s, m, ms, sl] = await Promise.all([
      supabase.from("candidates").select("id,selection_id,serial_number,test_number,pok_korp,panda,full_name,rank,nrp_nip,generation,unit_position,birth_place,birth_date,gender").is("deleted_at", null).order("serial_number"),
      supabase.from("exams").select("id,candidate_id,selection_id,exam_status,progress_percentage,kesum_classification,keswa_status,final_result,final_score"),
      ...
      supabase.from("medical_measurements").select("exam_id,candidate_id,height_cm,weight_kg,bmi,bmi_classification,chest_or_waist_lp,min_ideal_weight,max_ideal_weight"),
      ...
      supabase.from("selections").select("id,name,year_label,institution_header_line_1,institution_header_line_2").order("created_at", { ascending: false }),
    ]);
