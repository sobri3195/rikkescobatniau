...
import {
  parseNomorTesKesFile,
  buildKesTemplateCsv,
  type ParsedNomorTesRow,
} from "@/lib/import-nomor-tes/parse";
import {
  loadSelectionCandidatesForMatch,
  loadAttachmentCounts,
  matchRowsAgainstCandidates,
  type MatchedRow,
  type ExistingCandidate,
} from "@/lib/import-nomor-tes/match";
...
function ImportNomorTesPage() {
  const { roles } = useAuth();
  const allowed =
    roles.includes("super_admin") || roles.includes("admin") || roles.includes("registrasi");
  ...
  async function loadHistory() {
    ...
    const data = [...(db.test_number_import_sessions ?? [])]
      .sort((a: any, b: any) =>
  ...
  async function handleParse() {
    if (!file || !selId) {
      toast.error("Pilih seleksi dan file dulu");
      return;
    }
    setParsing(true);
    try {
      ...
      await logAudit({
        action: "parse_nomor_tes_file",
        module: "Import Nomor Tes",
        after: { file: file.name, sheet: sheet_used, total: rows.length },
      });
      // Match
      ...
      setMatched(m);
      await logAudit({
        action: "preview_import_nomor_tes",
        module: "Import Nomor Tes",
        after: { total: m.length },
      });
  ...
  function setManualMatch(rowIdx: number, candidateId: string | "skip") {
    ...
    logAudit({
      action: "manual_match_nomor_tes_row",
      module: "Import Nomor Tes",
      after: { rowIdx, candidateId },
    }).catch(() => {});
  }
  ...
  function bulkAction(
    ...
    setMatched((prev) =>
      prev.map((r) => {
        if (kind === "skip_ambiguous" && r.status === "ambiguous") {
          n++;
          return { ...r, status: "skipped", candidate: null };
        }
        if (kind === "skip_not_found" && r.status === "not_found") {
          n++;
          return { ...r, status: "skipped", candidate: null };
        }
        if (
          kind === "auto_pick_first_ambiguous" &&
          r.status === "ambiguous" &&
          r.candidate_options.length > 0
        ) {
          n++;
          return { ...r, candidate: r.candidate_options[0], confidence: "medium", status: "ready" };
        }
        if (kind === "promote_need_review" && r.status === "need_review" && r.candidate) {
          n++;
          return { ...r, status: "ready" };
        }
        if (kind === "reset_skipped" && r.status === "skipped") {
          n++;
          return { ...r, status: r.candidate ? "ready" : "not_found" };
        }
        return r;
      }),
    );
    logAudit({
      action: `bulk_manual_match_${kind}`,
      module: "Import Nomor Tes",
      after: { affected: n },
    }).catch(() => {});
    ...
    () =>
      matched.filter(
        (r) =>
          r.candidate &&
          r.status === "ready" &&
          (!r.conflict_other_kes_holder || (overrideDuplicate && isSuperAdmin)),
      ),
  ...
  function downloadErrorReport() {
    const errs = matched.filter((r) =>
      ["error", "ambiguous", "not_found", "duplicate_kes", "need_review"].includes(r.status),
    );
    if (!errs.length) {
      toast.info("Tidak ada error/warning.");
      return;
    }
    const aoa = [
      [
      ...
      ...errs.map((r) => [
        ...
        r.warnings.join("; "),
        r.status === "ambiguous"
          ? "Pilih kandidat manual di preview"
          : r.status === "not_found"
            ? "Verifikasi nama/tgl lahir, atau aktifkan Buat Peserta Baru"
            : r.status === "duplicate_kes"
              ? "Override hanya oleh Super Admin dengan alasan"
              : "",
      ]),
    ];
    ...
    downloadBlob(
      ...
      `import-nomor-tes-errors-${Date.now()}.xlsx`,
    );
    logAudit({
      action: "download_import_nomor_tes_error_report",
      module: "Import Nomor Tes",
    }).catch(() => {});
  }
  ...
  async function runImport() {
    ...
    if (toApply.length === 0 && toCreate.length === 0) {
      toast.error("Tidak ada baris yang siap diimpor");
      return;
    }
    setRunning(true);
    const startedAt = new Date().toISOString();
    try {
      // 1) Create local import session
      const db = getDb() as any;
      db.test_number_import_sessions = db.test_number_import_sessions ?? [];
      db.test_number_import_rows = db.test_number_import_rows ?? [];
      ...
      const session_id = generateId("tnis");
      db.test_number_import_sessions.push({
        id: session_id,
        ...sessionPayload,
      ...
      saveDb(db);
      await logAudit({
        action: "confirm_import_nomor_tes",
        module: "Import Nomor Tes",
        record_id: session_id,
        after: sessionPayload,
      });
      ...
      // 2) Apply updates to candidates in localDb
      for (const r of toApply) {
        ...
        if (updateAdmin) {
          if (r.no != null) {
            patch.serial_number = r.no;
            patch.sort_order = r.no;
          }
        ...
        try {
          updateCandidateLocal(r.candidate.id, patch);
          updated += 1;
          ...
          saveDb(rowDb);
          if (r.candidate.exam_id) recalcExamProgressLocal(r.candidate.exam_id);
          await logAudit({
            action: "update_candidate_nomor_tes",
            module: "Import Nomor Tes",
            candidate_id: r.candidate.id,
            record_id: r.candidate.id,
            before: before as never,
            after: { new_test_number: r.kes, new_test_number_status: "assigned" } as never,
          });
        } catch (error) {
          errors += 1;
          const rowDb = getDb() as any;
      ...
      for (const r of toCreate) {
        try {
          ...
          rowDb.test_number_import_rows.push({
            ...
            raw_data_json: r,
            error_messages_json: {
              error: error instanceof Error ? error.message : "Gagal membuat peserta",
            },
            created_at: nowIso(),
          });
          saveDb(rowDb);
      ...
      const doneDb = getDb() as any;
      const session = (doneDb.test_number_import_sessions ?? []).find(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Hash className="h-6 w-6" /> Import Nomor Tes Final
          </h1>
          <p className="text-sm text-muted-foreground">
            Upload file <strong>KES</strong> (XLSX/CSV) untuk mengisi Nomor Tes final pada peserta
            yang sudah ada — Rontgen, EKG, attachment, exam, dan progress tetap utuh.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            onClick={() => downloadBlob(buildKesTemplateCsv(), "template-nomor-tes-kes.csv")}
          >
      ...
      <Tabs defaultValue="wizard">
        <TabsList>
          ...
          <TabsTrigger value="history">
            <History className="h-4 w-4 mr-1" /> Riwayat Import
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="wizard" className="space-y-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">1. Pilih Seleksi & Upload File</CardTitle>
            </CardHeader>
          ...
          {matched.length > 0 && (
            <>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">2. Ringkasan & Opsi</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    <Badge className={STATUS_BADGE.duplicate_kes}>
                      Duplikat KES: {totals.duplicate_kes}
                    </Badge>
                    ...
                    {isSuperAdmin && (
                      <label className="flex items-center gap-2">
                        ...
                        <span className="text-rose-700">
                          Override Nomor Tes duplikat (Super Admin)
                        </span>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2 border-t">
                    ...
                    <div className="flex flex-wrap items-center gap-1 ml-auto">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => bulkAction("auto_pick_first_ambiguous")}
                        disabled={totals.ambiguous === 0}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => bulkAction("skip_ambiguous")}
                        disabled={totals.ambiguous === 0}
                      >
                        Skip Ambiguous
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => bulkAction("skip_not_found")}
                        disabled={totals.not_found === 0}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => bulkAction("promote_need_review")}
                        disabled={totals.need_review === 0}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => bulkAction("reset_skipped")}
                    ...
                    >
                      <ShieldCheck className="h-4 w-4 mr-2" /> Jalankan Import (
                      {toApply.length + toCreate.length})
                    </Button>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {filtered.map((r) => {
                          ...
                          const hasMedical =
                            !!(
                              r.candidate?.radiology_initial_status &&
                              r.candidate.radiology_initial_status !== "Belum Diisi"
                            ) ||
                            !!(
                              r.candidate?.ekg_initial_status &&
                              r.candidate.ekg_initial_status !== "Belum Diisi"
                            ) ||
                            r.rad_attachments_count > 0 ||
                            r.ekg_attachments_count > 0;
                          return (
                            <tr
                              key={`${r.source_row_number}-${idx}`}
                            ...
                            >
                              ...
                              <td className="p-2">
                                <Badge variant="outline" className={CONFIDENCE_BADGE[r.confidence]}>
                                  {r.confidence}
                                </Badge>
                              </td>
                              ...
                              <td className="p-2">
                                <div>
                                  RO: {r.candidate?.radiology_initial_status ?? "—"} (
                                  {r.rad_attachments_count})
                                </div>
                                <div>
                                  EKG: {r.candidate?.ekg_initial_status ?? "—"} (
                                  {r.ekg_attachments_count})
                                </div>
                                {hasMedical && (
                                  <Badge
                                    variant="outline"
                                    className="bg-emerald-50 text-emerald-700 text-[10px] mt-1"
                                  >
                                    Data RO/EKG dipertahankan
                                  </Badge>
                                )}
                              </td>
                              ...
                              <td className="p-2">
                                ...
                                {r.conflict_other_kes_holder && (
                                  <div className="text-rose-700">
                                    KES dipakai: {r.conflict_other_kes_holder.full_name}
                                  </div>
                                )}
                              </td>
                              <td className="p-2">
                                {r.status === "ambiguous" && r.candidate_options.length > 0 && (
                                  <Select onValueChange={(v) => setManualMatch(idx, v as never)}>
                                    <SelectTrigger className="h-7 w-44 text-[11px]">
                                      <SelectValue placeholder="Pilih candidate…" />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                {r.candidate?.exam_id && (
                                  ...
                                  >
                                    <Link
                                      to="/rikkes/$id"
                                      params={{ id: r.candidate.exam_id ?? r.candidate.id }}
                                      search={{
                                        from: "import-nomor-tes",
                                        candidateId: r.candidate.id,
                                        selectionId: r.candidate.selection_id,
                                      }}
                                    >
            ...
            <Card>
              <CardContent className="p-4 space-y-2">
                ...
                <div className="text-sm">
                  Updated: <strong>{result.updated}</strong> • Skipped:{" "}
        ...
        <TabsContent value="history">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Riwayat Import Nomor Tes</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Pastikan ringkasan di bawah sudah sesuai. Aksi ini akan mengupdate Nomor Tes pada
              peserta yang sudah ada.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 text-sm">
            <div>
              Total baris file: <strong>{totals.total}</strong>
            </div>
            <div>
              Siap update Nomor Tes: <strong className="text-emerald-700">{toApply.length}</strong>
            </div>
            {createMissing && (
              <div>
                Akan dibuat peserta baru: <strong>{toCreate.length}</strong>
              </div>
            )}
            <div>
              Perlu review (low confidence): <strong>{totals.need_review}</strong>
            </div>
            <div>
              Ambiguous: <strong>{totals.ambiguous}</strong>
            </div>
            <div>
              Not Found: <strong>{totals.not_found}</strong>
            </div>
            <div>
              Duplikat KES: <strong>{totals.duplicate_kes}</strong>
            </div>
            <div className="text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded p-2">
              ✓ Data Rontgen, EKG, attachment, exam, section, stage, dan progress yang sudah ada{" "}
              <strong>akan dipertahankan</strong>.
            </div>
            <label className="flex items-start gap-2 pt-2 cursor-pointer">
              <Checkbox
                checked={confirmAck}
                onCheckedChange={(b) => setConfirmAck(!!b)}
                className="mt-0.5"
              />
              <span className="text-xs">
                Saya memahami bahwa sistem hanya akan mengupdate Nomor Tes dan data administratif
                yang dipilih, tanpa menghapus Rontgen, EKG, attachment gambar, exam, section, stage,
                dan progress yang sudah ada.
              </span>
            </label>
          </div>
          <DialogFooter>
